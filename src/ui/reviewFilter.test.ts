...
import { hasFailedRequirements, counts, tier, matchesFailuresOnly, compareForFailuresMode } from "./reviewFilter.ts";
...
Deno.test("tier groups failures and critical together", () => {
  const failed = item({ requirements: [{ requirement_summary: "A", satisfied: false, failure_summary: "x" }] });
  const critical = item({ issues: [{ severity: 'CRITICAL', description: 'x' }] });
