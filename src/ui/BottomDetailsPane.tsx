...
export default function BottomDetailsPane({
  ...
  if (!issues || issues.length === 0) {
    lines.push(<Text key="iss-none" color="gray">No tool issues recorded</Text>);
  } else {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      const body = showSummaries ? (it.summary || it.description) : it.description;
      lines.push(
        <Text key={`iss-${i}`}>
  ...
  if (requirements && requirements.length > 0) {
    ...
    lines.push(
      <Text key="req-h">
        Requirements — <Text color={satisfied === requirements.length ? "green" : "red"}>{satisfied}/{requirements.length} satisfied</Text>
      </Text>,
    );
    const list = showSummaries ? requirements.filter((r) => !r.satisfied) : requirements;
  ...
  return (
    <Box flexDirection="column" height={height}>
      ...
      {overflowBottom ? <Text color="gray">… +{bottomRemaining} more (scroll)</Text> : null}
    </Box>
  );
}
